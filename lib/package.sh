...
serve_packages() {
    ...
    if source "${LIB_DIR}/qr.sh" 2>/dev/null; then
        local qr_png="${PACKAGES_DIR}/heartbeat_qr.png"
        local serve_url="http://${SERVER_IP}:${port}"
        if save_qr_png "$serve_url" "$qr_png" 2>/dev/null && [[ -s "$qr_png" ]]; then
            qr_section='<div class="qr-section"><img class="qr-img" src="heartbeat_qr.png" alt="QR code"><p class="qr-label">Scan to open on your phone</p><p class="qr-hint">Point your camera at the code above</p></div>'
        fi
    fi
